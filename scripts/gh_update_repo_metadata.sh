...
# ── Description ──────────────────────────────────────────────────────
DESCRIPTION="Asset-Aware MCP Server — AI Agent precisely accesses tables, figures, sections from PDFs + .docx round-trip editing (DFM) with 48 tools / 13 resources, segmentation export, layout overlay, OCR preprocessing, knowledge graph (LightRAG)"
