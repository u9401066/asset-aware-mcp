...
- Completed a real Asset-Aware folder smoke for `/home/eric/workspace251226/asset-aware-mcp/一般案-新案2025.00 (20250520)_20260128R` covering 26 files (18 DOCX, 5 DOC, 3 PDF). Source files were not modified; the first full report is in `tmp/asset_aware_real_roundtrip_20260511T144132Z/reports/report.json`.
- Fixed real-world DOCX/DFM round-trip regressions found by that smoke: Markdown table blank rows are preserved, break blocks are protected from split-format text edits, terminal line-break normalization no longer trips save guards, break-only runs do not get duplicate text, and PDF content DOCX conversion re-encodes python-docx-unrecognized JPEG headers.
- Re-ran the previously failing real files successfully: five Word files now pass strict validator and LibreOffice raster layout checks with max mean diff `0.0`, and the PDF with an Adobe APP14 JPEG now produces `converted_from_pdf.docx` with the figure embedded. Fix-check report: `tmp/asset_aware_real_roundtrip_fixcheck_20260511T150045Z/reports/report.json`.
- Re-ran the full real folder after fixes: all 26 files processed, source `changed_count=0`, Word status `21 pass / 2 validator_pass_layout_diff` where both diffs are legacy `.doc` source-conversion drift and converted-DOCX-to-IR rebuild diff is `0.0`, and all 3 PDFs passed citation artifact checks. Full after-fix report: `tmp/asset_aware_real_roundtrip_full_afterfix_20260511T153916Z/reports/report.json`.
- Strengthened citation-ready PDF provenance by adding `source_pdf_sha256` and `selected_page_map` to `DocumentManifest`, with PyMuPDF and Marker ingest regression assertions.
- Added Foam/LLM wiki promotion output to `citation_bundle`: `output_format="foam"` with optional `citation_key` now emits YAML frontmatter, `^spn-...` block anchors, wikilink/embed strings, verification state, locator hashes, quote hashes, and embedded AssetRef JSON; `evidence(op="bundle")` preserves these Foam options.
- Added Foam wiki write/maintenance support: `citation_bundle(..., wiki_root=..., output_path=..., index_path=...)` writes Foam evidence notes and updates a managed evidence index block, while `evidence(op="health", wiki_root=...)` validates embedded span AssetRefs plus `[[note#^spn-...]]` target notes/anchors.
- Added Table/Figure Foam note promotion: `document_asset(op="foam_notes", ...)` writes `table_evidence` / `figure_evidence` notes from manifest assets, preserving source block/order, line spans, section context, source PDF hash, asset locator hash, and embedded table/figure AssetRef JSON. Foam health now verifies span/table/figure AssetRefs and generic `[[note#^...]]` target anchors.
- Added DOCX locator metadata for DFM blocks: paragraphs, images, fields, headers/footers, footnotes, tables, and nested tables now preserve Word source part/story/element, paragraph/table/source indexes, run ranges, text char/byte/hash locator, and table cell locators in `DfmBlock.metadata`; block reads, block listings, split `format.yaml`, and `revisions.jsonl` expose the same locator contract for citation promotion/audit.
...
- `v0.6.29` local release gates passed: `uv run ruff check .`, `uv run ruff format --check .`, `uv run mypy src --ignore-missing-imports`, `uv lock --check`, full `uv run pytest -q` (`840 passed, 21 skipped`), docs site check, `node --check`, release harness audit, `./scripts/count_tools.sh`, VSIX `sync-assets:check`, VSIX `test:ci` (`119 passing` plus package contents), VSIX install/update smoke, `uv build`, VSIX package, release artifact audit, Docker build/import smoke, and `git diff --check`.
- Dockerfile release smoke compatibility was improved by removing the BuildKit-only cache mount from dependency installation; the image now builds with this host's legacy Docker builder and still imports `src.presentation.server`.
...
- Improved the docs site for human UX: added task cards to the overview, added `Design And UX Notes` to both Pages and GitHub Wiki, documented the site IA/layout/completeness contract, refined the visual palette, and verified `#/design-ux`, `site-content/design-ux.md`, and the GitHub Wiki design page all return HTTP 200.
...
- 0.6.29 exact-path segmented commits, push, GitHub Pages verification, and annotated `v0.6.29` tag.
