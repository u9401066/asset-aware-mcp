"""Run explicit native Table generation through an actual isolated Codex CLI."""
...
from tests.codex_table_expansion.scenario import prompt, template
...
def main():
    ...
    native = workspace / "template.xlsx"
    ...
    cli[-1:-1] = [
        "-c",
        'mcp_servers.asset_aware_under_test.enabled_tools=["document","table_data","table_manage"]',
    ]
    status = execute(cli, text, output, 900)
    from tests.codex_table_expansion.audit import write_audit
