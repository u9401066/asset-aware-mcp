...
- 📄 **Asset-Aware ETL** - PDF → Markdown with a PyMuPDF-first parser and retained Marker code path:
  ...
  - **Marker** (`use_marker=True`) - High-precision structured parsing code path retained, but packaged runtime remains on security hold in v0.6.34 until upstream `marker-pdf` supports patched Pillow
...
# v0.6.34: Marker extra is temporarily empty because marker-pdf pins
...
- Marker backend: temporarily disabled in v0.6.34 because `marker-pdf` pins vulnerable `Pillow<11`; the `marker` / `pdf` extras are compatibility placeholders until upstream supports patched Pillow.
