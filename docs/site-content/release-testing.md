...
換成新版解碼結果。回歸案例涵蓋來源／保存檔損壞、symlink、完整目錄、鎖定、
尺寸／色彩政策、舊引用與目前更新檢查。CI 的 Python3.10 工作另裝隔離舊解碼器。

同一 NIST 測試流程在新實作以預設 Codex 重跑，292.81 秒完成 175 次成功呼叫、
零工具錯誤，仍有 15 張影格 PNG、一張區域預覽、三個 TIFF 版本、六格字串與
三份 Wiki。第一次稽核不接受額外的合法工作簿結構讀取，第二次未辨識完整
全域 contract；兩次失敗保留。補上完整工作簿來源／分頁核對與全域能力檢查後，
第三次稽核通過，仍要求每個修改操作的完整 schema、receipt 與實際影像。
最新完整套件 3,682 項通過、33 項選配略過（706.74 秒）；後補稽核與保存範圍
共 69 項通過（2.59 秒）。下列 258 次呼叫是前一批基線，兩批證據均保留。
...
補入 Python 3.10 的 CI 清單後，balanced／compact 兩個完整批註案例均超過原先
180 秒上限；該輪 **1,157 項通過、4 項略過、2 項逾時失敗**的紀錄保留。
隔離 Python 3.10 診斷以原有全部斷言完成 **408 次 MCP 呼叫、142.51 秒**，確認
流程可以完成。只有這兩個案例的上限改為 300 秒，所屬 CI 工作上限改為 20 分鐘，
並輸出最慢案例時間；其他案例維持原上限。程式、分頁完整性與內容核對條件均未改動。
調整後，隔離 Python 3.10 的正式 pytest 兩種設定皆通過（**2 項，266.20 秒**）。
