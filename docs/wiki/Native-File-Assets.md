...
依原意修正目前版本的公式，再重新計算與核對。公式輸入使用
`value.kind="formula"`，`value.value` 由 `=` 開始；不要把原生 XML 的 `of:=`
前綴直接當成輸入。新增／刪除／重排工作表與列欄
...
轉錄、語意、版面及公式結果。範例與實測見 [Release And Testing](Release-And-Testing)。
