...
- 📄 **資產感知 ETL** - PDF → Markdown，以 PyMuPDF 為預設解析路徑並保留 Marker code path：
  ...
  - **Marker**（`use_marker=True`）- 高精度結構化解析 code path 保留；v0.6.30 packaged runtime 仍因 upstream `marker-pdf` 尚未支援 patched Pillow 而暫時 security hold
...
# v0.6.30：Marker extra 暫時停用；marker-pdf 1.10.2 鎖 Pillow<11，
...
安裝建議：
- 預設安裝：`uv sync`
- Marker backend：v0.6.30 暫時停用；`marker` / `pdf` extras 保留名稱但不安裝 `marker-pdf`。
