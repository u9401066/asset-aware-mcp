...
def calc_profile(root: Path, request: NativeWorkbookRendition) -> str:
...
<prop oor:name="OOXMLRecalcMode" oor:op="fuse"><value>{recalc}</value></prop></item>
    ...
    def convert(
        ...
        with tempfile.TemporaryDirectory(prefix="native-workbook-render-") as directory:
            ...
            if not pdf.is_file() or pdf.is_symlink():
                raise ValueError(
                    "LibreOffice produced no workbook PDF; check that Calc is installed and can open this XLSX"
                )
