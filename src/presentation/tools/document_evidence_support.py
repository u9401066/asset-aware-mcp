...
from src.application.etl_references import (
    _ASSET_LOCATOR_VERSION as _ASSET_LOCATOR_VERSION,
)
from src.application.etl_references import (
    _asset_locator_payload as _asset_locator_payload,
)
from src.application.etl_references import (
    _asset_locator_sha256 as _asset_locator_sha256,
)
from src.application.etl_references import (
    _asset_ref_from_manifest_asset as _asset_ref_from_manifest_asset,
)
from src.application.etl_references import (
    _strict_json_equal as _strict_json_equal,
)
from src.application.etl_references import (
...
def _verify_span_ref_payload(ref: dict[str, Any], *, repository: Any) -> dict[str, Any]:
    doc_id = str(ref.get("doc_id") or "")
    span_id = str(ref.get("span_id") or "")
    spans = (
        load_or_build_evidence_spans(repository, doc_id)
        if doc_id and span_id and ref.get("source_type") == "span"
        else []
    )
    return verify_span_reference(
        ref, next((s for s in spans if s.span_id == span_id), None)
    )
...
def _verify_asset_ref_payload(
    ref: dict[str, Any], *, repository: Any
) -> dict[str, Any]:
    if ref.get("source_type") == "span" and ref.get("canonical_asset_ref") is not False:
        return _verify_span_ref_payload(ref, repository=repository)
    doc_id = ref.get("doc_id")
    manifest = (
        repository.load_manifest(doc_id) if isinstance(doc_id, str) and doc_id else None
    )
    return verify_asset_reference(ref, manifest)
